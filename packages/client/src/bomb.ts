...
export class BombManager {
  ...
  private readonly patternSet: Set<string>;

  constructor(private readonly pattern: [number, number][]) {
    this.patternSet = new Set(pattern.map(([dx, dy]) => `${dx},${dy}`));
  }
    ...
    while (queue.length > 0) {
      ...
      if (!isWall) {
        for (const [ndx, ndy] of [[dx+1,dy],[dx-1,dy],[dx,dy+1],[dx,dy-1]] as [number,number][]) {
          const key = `${ndx},${ndy}`;
          if (!visited.has(key) && this.patternSet.has(key)) {
            visited.add(key);
            queue.push([ndx, ndy]);
          }
        }
  ...
  getEntities(): BombEntity[] { return this.entities; }
}
