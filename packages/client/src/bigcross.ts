...
export class BigCrossManager {
  ...
  private triggerExplosion(e: BigCrossEntity, terrain: Terrain): void {
    ...
    this.applyExplosion(e.cells, terrain);
  }

  /**
   * Compute the infinite cross: travel in each cardinal direction until a
   * border or hard wall blocks the arm. Soft (interior) blocks are included
   * as the final cell in that arm and are destroyed.
   */
  private computeCells(tileX: number, tileY: number, terrain: Terrain): [number, number][] {
    ...
    for (const [dc, dr] of [[1, 0], [-1, 0], [0, 1], [0, -1]] as [number, number][]) {
      ...
      while (r >= 0 && r < rows && c >= 0 && c < cols && steps < this.maxRange) {
        ...
        if (isBorder) break;                // permanent border — stop, don't include
        cells.push([c, r]);
        if (isStone(terrain, c, r)) break; // soft block — include it, then stop
        c += dc; r += dr;
        steps++;
      }
    }
  ...
  private applyExplosion(cells: [number, number][], terrain: Terrain): void {
